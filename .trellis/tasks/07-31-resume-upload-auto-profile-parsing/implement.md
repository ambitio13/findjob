...
- [ ] Create `backend/app/schemas/resume_facts.py`:
...
- [ ] Add a prompt marker constant + a deterministic fake output in
...
- [ ] **Gate**: `python3 -m pytest app/tests/test_model_gateway.py -q` passes;
      new fake route returns a valid `ResumeFactsModelOutput`.

## 2. Extraction executor + prompt

- [ ] Create `backend/app/agents/prompts/resume_fact.py`:
...
- [ ] Create `backend/app/agents/resume_fact_executor.py`:
...
- [ ] `_fail_run` helper mirrors `jd_analysis_service.py:277-307` for
...
- [ ] `persist_outputs` writes `parsed_facts.facts` (validated object) +
...
- [ ] **Gate**: new `test_resume_fact_service.py` covering success (asserts
...
- [ ] `POST /api/v1/resumes` (`resumes.py:34-114`): add
...
- [ ] `POST /api/v1/resumes/{resume_id}/versions/{version_id}/extract`:
...
- [ ] Update response schemas to surface extraction status (reuse
      `ResumeVersionOut.parsed_facts`; optionally add
      `extraction_status`/`extraction_run_id` derived fields).
- [ ] **Gate**: extend `app/tests/test_resume_upload.py` — upload now yields
...
- [ ] Prompt builder `build_resume_fact_messages` /
      ...
      provenance metadata.
- [ ] Update / extend `app/tests/test_jd_analysis_contracts.py` prompt +
...
- [ ] **Gate**: `python3 -m pytest app/tests/test_jd_analysis_contracts.py
      app/tests/test_jd_analysis_api.py -q` passes.

## 6. Frontend: extraction status + facts + draft review

- [ ] `frontend/src/pages/resumes/ResumeDetailPage.tsx`: show extraction
      ...
      endpoint.
- [ ] Profile draft review: surface draft fields derived from `facts` as a
      read-only preview. The apply/merge action belongs to
      `07-31-structured-profile-text-fields`.
- [ ] API client `frontend/src/api/client.ts`: add
      `reextractResumeFacts(resumeId, versionId)`.
- [ ] **Gate**: `cd frontend && npm run lint && npm run type-check && npm run build`
      pass.

## 7. Full quality gate

- [ ] `cd backend && python3 -m pytest -q` — all green.
...
- [ ] Run `trellis-check` skill before commit.
