...
- [ ] 读取 frontend `components.md`、`api-integration.md`、`state-management.md`、
  `css-layout.md`、`type-safety.md`、`quality.md` 和 shared `typescript.md`。
- [ ] 审阅当前未提交 diff，确认它属于本任务：
  - `needs_review` 可人工继续 prepare。
  - `BossInspectEntry` 打破“必须先有 application 才能读取当前职位”的依赖。
- [ ] 补齐 `BossInspectEntry` 的类型、错误状态、空简历状态和 bridge 未连接状态。
- [ ] 确认 `RecommendedJobPilotPanel` 在 `communicate` 和 `needs_review` 下的按钮门控正确：
  - `skip` 不能继续 prepare。
  - `needs_review` 必须人工点击后才能继续。
  - approve 和 execute 始终人工触发。
- [ ] 确认 semi-auto 只自动执行 inspect → match → prepare，不自动 approve/execute。
- [ ] 确认切换 application、重新开始、API 失败时不会复用旧 action/result。
- [ ] 确认失败状态展示 `agent_run_id` 或可定位错误信息，不暴露 HR 消息之外的敏感内容。
- [ ] 根据风险补前端测试；如果当前项目没有前端测试框架，至少用 `pnpm lint/type-check/build`
  和浏览器手动检查记录替代。
- [ ] 提交时只包含本任务前端文件和必要任务文件，不混入其它横切任务。
